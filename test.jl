...
function plot_points!(
    points::Vector{<:Vector{<:Real}}=generate_points_on_sphere(rand(1:10)); 
    radius=5)
    plotly()
    scatter3d!([Tuple(i) for i in points], color="#ff884b", markersize=radius, label="Points on Sphere", markerstrokecolor="#283d53")
    scatter!()
    xlabel!("X")
    ylabel!("Y")
    zlabel!("Z")
...
#-------------------------------------
function main(Number_of_points::Int=4, iterations::Int=100, force_factor::Real=1, show_evolution::Bool=true, min_time::Real=10, radius::Real=1)
...
n = 5
points = main(n)
evolve_space!(points, 40, 0.75, 1, true, 20)
#------------------------------------------------
