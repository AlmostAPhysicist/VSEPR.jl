...
# Fields
    ...
    #Kinetics
  -  `position` :: A `Point2` or `Point3`
  -  `velocity` :: A `Point2` or `Point3`
  -  `acceleration` :: A `Point2` or `Point3`
...
mutable struct Particle
    ...
    function Particle(position::Point3=Point3f(0), velocity::Point3=Point3f(0), acceleration::Point3=Point3f(0);
        mass::Real=1, charge::Real=1, color::String=rand(BASE_COLORS),
        shape=UnitSphere,
        size::Real=5, alpha::Real=1)
        new(Observable(position), velocity, acceleration, mass, charge, Observable(color), Observable(shape), Observable(size), Observable(alpha))
    end

    function Particle(position::Point2=Point2f(0), velocity::Point2=Point2f(0), acceleration::Point2=Point2f(0);
        mass::Real=1, charge::Real=1, color::String=rand(BASE_COLORS),
